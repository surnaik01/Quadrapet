...
static const double zoom_increment = 0.02;  // ratio of single click-wheel zoom increment to
                                            // vertical extent model and data
static mjModel *m = NULL;
static mjData *d = NULL;
// strings
static char filename[kBufSize] = "";
static char previous_filename[kBufSize] = "";
static char loadError[kBufSize] = "";
...
// file section of UI
static const mjuiDef defFile[] = {{mjITEM_SECTION, "File", 1, NULL, "AF"},
...
// watch section of UI
static const mjuiDef defWatch[] = {{mjITEM_SECTION, "Watch", 0, NULL, "AW"},
...
static std::recursive_mutex latest_command_lock_;
...
static void profilerupdate(void) {
  ...
  float tdata[5] = {(float)(total / number),
                    ...
                    (float)(d->timer[mjTIMER_CONSTRAINT].duration / number), 0};
  ...
  float sdata[6] = {(float)m->nv,   (float)m->nbody,
                    (float)d->nefc, (float)mju_sqrt((mjtNum)d->solver_nnz),
                    (float)d->ncon, (float)d->solver_iter};
...
static void profilershow(mjrRect rect) {
  mjrRect viewport = {rect.left + rect.width - rect.width / 4, rect.bottom, rect.width / 4,
                      rect.height / 4};
...
static void infotext(char (&title)[kBufSize], char (&content)[kBufSize], double interval) {
  ...
  mju::sprintf_arr(
      ...
      "%-9.3f %s%d x\n%d  (%d con)\n%.3f\n%.1f  (%d "
      "it)\n%.0f\n%.3f\n%.3f\n%.3f",
      d->time, realtime_nominator.c_str(), settings.slow_down, d->nefc, d->ncon,
      settings.run
      ...
      solerr, d->solver_iter, 1 / interval, d->maxuse_stack / (double)d->nstack,
      d->maxuse_con / (double)m->nconmax, d->maxuse_efc / (double)m->njmax);
...
static void watch(void) {
  ...
  mju::strcpy_arr(ui0.sect[SECT_WATCH].item[2].multi.name[0], "invalid field");
...
#define X(TYPE, NAME, NR, NC)                                                           \
  if (!mju::strcmp_arr(#NAME, settings.field) && !mju::strcmp_arr(#TYPE, "mjtNum")) {   \
...
static void makerendering(int oldstate) {
  ...
  mjuiDef defRendering[] = {{mjITEM_SECTION, "Rendering", oldstate, NULL, "AR"},
...
static void makejoint(int oldstate) {
  ...
  mjuiDef defJoint[] = {{mjITEM_SECTION, "Joint", oldstate, NULL, "AJ"}, {mjITEM_END}};
  ...
  for (i = 0; i < m->njnt && itemcnt < mjMAXUIITEM; i++)
    if ((m->jnt_type[i] == mjJNT_HINGE || m->jnt_type[i] == mjJNT_SLIDE)) {
      ...
      if (m->names[m->name_jntadr[i]]) {
        mju::strcpy_arr(defSlider[0].name, m->names + m->name_jntadr[i]);
      } else {
      ...
      // set range
      if (m->jnt_limited[i])
        mju::sprintf_arr(defSlider[0].other, "%.4g %.4g", m->jnt_range[2 * i],
                         m->jnt_range[2 * i + 1]);
...
static void makecontrol(int oldstate) {
  ...
  for (i = 0; i < m->nu && itemcnt < mjMAXUIITEM; i++) {
    ...
    if (m->names[m->name_actuatoradr[i]]) {
      mju::strcpy_arr(defSlider[0].name, m->names + m->name_actuatoradr[i]);
    } else {
    ...
    if (m->actuator_ctrllimited[i])
      mju::sprintf_arr(defSlider[0].other, "%.4g %.4g", m->actuator_ctrlrange[2 * i],
...
static mjtNum timer(void) { return (mjtNum)(1000 * glfwGetTime()); }
...
static void copycamera(mjvGLCamera *camera) {
  ...
  mju::sprintf_arr(clipboard,
                   ...
                   (camera[0].pos[2] + camera[1].pos[2]) / 2, cam_right[0], cam_right[1],
                   cam_right[2], camera[0].up[0], camera[0].up[1], camera[0].up[2]);
...
static void drop(GLFWwindow *window, int count, const char **paths) {
  ...
  if (count > 0) {
    mju::strcpy_arr(filename, paths[0]);
...
static void loadmodel(void) {
  ...
  if (mju::strcmp_arr(filename, previous_filename)) {
    alignscale();
    mju::strcpy_arr(previous_filename, filename);
  }
...
// to be written in Objective-C, we separate it into a different source file.
#ifdef __APPLE__
...
static void uiEvent(mjuiState *state) {
  ...
  if ((state->dragrect == ui0.rectid) || (state->dragrect == 0 && state->mouserect == ui0.rectid) ||
      ...
      switch (it->itemid) {
        ...
        case 9:  // Full screen
          if (glfwGetWindowMonitor(window)) {
            ...
            glfwSetWindowMonitor(window, NULL, windowpos[0], windowpos[1], windowsize[0],
                                 windowsize[1], 0);
          }
          ...
          // currently windowed: switch to full screen
            ...
            // switch
            glfwSetWindowMonitor(window, glfwGetPrimaryMonitor(), 0, 0, vmode.width, vmode.height,
                                 vmode.refreshRate);
          }
  ...
  if ((state->dragrect == ui1.rectid) || (state->dragrect == 0 && state->mouserect == ui1.rectid) ||
      state->type == mjEVENT_KEY) {
    // process UI event
  ...
  if (state->type == mjEVENT_KEY && state->key != 0) {
    switch (state->key) {
      ...
      case ']':  // cycle up fixed cameras
        if (m && m->ncam) {
          cam.type = mjCAMERA_FIXED;
          // settings.camera = {0 or 1} are reserved for the free and tracking
          // cameras
  ...
  if (state->type == mjEVENT_PRESS && state->mouserect == 3 && m) {
    ...
    if (state->doubleclick) {
      ...
      mjtNum selpnt[3];
      int selgeom, selskin;
      int selbody = mjv_select(m, d, &vopt, (mjtNum)r.width / (mjtNum)r.height,
                               (mjtNum)(state->x - r.left) / (mjtNum)r.width,
                               (mjtNum)(state->y - r.bottom) / (mjtNum)r.height, &scn, selpnt,
                               &selgeom, &selskin);
  ...
  if (state->type == mjEVENT_MOVE && state->dragrect == 3 && m) {
    ...
    mjrRect r = state->rect[3];
    if (pert.active)
      mjv_movePerturb(m, d, action, state->dx / r.height, -state->dy / r.height, &scn, &pert);
    else
      mjv_moveCamera(m, action, state->dx / r.height, -state->dy / r.height, &scn, &cam);
...
static void prepare(void) {
  ...
  // update control
  if (settings.ui1 && ui1.sect[SECT_CONTROL].state) {
    mjui_update(SECT_CONTROL, -1, &ui1, &uistate, &con);
  }
...
static void render(GLFWwindow *window) {
  ...
  if (!settings.run || settings.loadrequest) {
    mjr_overlay(mjFONT_BIG, mjGRID_TOPRIGHT, smallrect, settings.loadrequest ? "loading" : "pause",
                NULL, &con);
  }

  // show realtime label
  if (settings.run && settings.slow_down != 1) {
    std::string realtime_label = "1/" + std::to_string(settings.slow_down) + " x";
    mjr_overlay(mjFONT_BIG, mjGRID_TOPRIGHT, smallrect, realtime_label.c_str(), NULL, &con);
  }
  ...
  // show info
  if (settings.info) {
    mjr_overlay(mjFONT_NORMAL, mjGRID_BOTTOMLEFT, rect, info_title, info_content, &con);
  }
  ...
  while (!settings.exitrequest) {
    ...
    if (m) {
      ...
      if (settings.run) {
        ...
        // inject noise
        if (settings.ctrlnoisestd) {
          // convert rate and scale to discrete time given current timestep
          mjtNum rate = mju_exp(-m->opt.timestep / settings.ctrlnoiserate);
          mjtNum scale = settings.ctrlnoisestd * mju_sqrt(1 - rate * rate);
        ...
        mjtNum offset = mju_abs((d->time * settings.slow_down - simsync) - (tmstart - cpusync));
        if (d->time * settings.slow_down < simsync || tmstart < cpusync || cpusync == 0 ||
            offset > syncmisalign * settings.slow_down || settings.speed_changed) {
          // re-sync
          cpusync = tmstart;
          simsync = d->time * settings.slow_down;
          settings.speed_changed = false;
        ...
        // in-sync
          ...
          while ((d->time * settings.slow_down - simsync) < (glfwGetTime() - cpusync) &&
...
static void init(void) {
  ...
  // create window
  window = glfwCreateWindow((2 * vmode.width) / 3, (2 * vmode.height) / 3, "Simulate", NULL, NULL);
  if (!window) {
    glfwTerminate();
...
static bool gui_is_alive() {
  return !glfwWindowShouldClose(mujoco_interactive::window) &&
         !mujoco_interactive::settings.exitrequest;
}
...
static void start_simulation() { simulate_thread_ = std::make_shared<std::thread>(simulate); }

static void join_simulation() { simulate_thread_->join(); }
...
static void update_ctrl_() {
  ...
  if (latest_command_.kp.size() == 0) {
    // std::cout << "WARNING: update_ctrl called with no command recorded" <<
    // std::endl;
    return;
  }
  if (actuator_models_.size() != m->nu) {
    throw std::runtime_error(
        "Number of actuator models != number of actuators "
        "determined by Mujoco");
  }
  for (int i = 0; i < m->nu; i++) {
  ...
  // std::cout << "act vel: " << actuator_velocities() << std::endl;
  // std::cout << "ctrl: " << std::vector<double>(d->ctrl, d->ctrl + 12)
  //           << std::endl;
}
...
static void calibrate_motors_blocking() {
  ...
  command.kp.assign(12, 0.0);
  command.kd.assign(12, 0.0);
  command.position_target.assign(12, 0.0);
  command.velocity_target.assign(12, 0.0);
  command.feedforward_torque.assign(12, 0.0);
  set_actuator_command(command);
